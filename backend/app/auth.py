...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security), db: AsyncSession = Depends(database.get_db)):
    try:
...
async def login(form_data: dict, db: AsyncSession = Depends(database.get_db)):
    ...
    return {"access_token": token, "token_type": "bearer", "is_premium": user.is_premium}