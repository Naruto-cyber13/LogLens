...
async def upload_logs(
    ...
    forbidden_counts_by_ip: Dict[str, int] = {}    # 403 → forbidden spam
    not_found_counts_by_ip: Dict[str, int] = {}    # 404 → scanning / enumeration
    ...
    for line in lines:
        ...
        if parsed:
            ...
            status_code_str = parsed.get("status")
        ...
        if status_code_str and ip:
            try:
                status_code = int(status_code_str)
                if status_code == 401:
                    failed_counts_by_ip[ip] = failed_counts_by_ip.get(ip, 0) + 1

        # 403 → forbidden spam evidence
                elif status_code == 403:
                    forbidden_counts_by_ip[ip] = forbidden_counts_by_ip.get(ip, 0) + 1

        # 404 → scanning / enumeration evidence
                elif status_code == 404:
                    not_found_counts_by_ip[ip] = not_found_counts_by_ip.get(ip, 0) + 1
    ...
    brute_force_ips = [ip for ip, cnt in failed_counts_by_ip.items() if cnt > 4]
    for ip in brute_force_ips: 
        top_threats["brute_force"] = top_threats.get("brute_force", 0) + 1
        threats_count += 1
        suspicious_ips_set.add(ip)
    '''if brute_force_ips:
    ...
    # 403 Forbidden spam detection
    # --------------------------------------------------
    for ip, count in forbidden_counts_by_ip.items():
        if count >= 3:
            top_threats["forbidden_spam"] = top_threats.get("forbidden_spam", 0) + 1
            threats_count += 1
            suspicious_ips_set.add(ip)
    # 404 Not Found spam detection (scanning / enumeration)
    # --------------------------------------------------
    for ip, count in not_found_counts_by_ip.items():
        if count >= 5:
            top_threats["not_found_spam"] = top_threats.get("not_found_spam", 0) + 1
            threats_count += 1
            suspicious_ips_set.add(ip)
