...
export default function Register({ onSwitchToLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await register(email, password, isPremium);
    if (!result.success) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <h1 className="text-7xl font-bold bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400 mb-2">
            LogLens
          </h1>
          <p className="text-slate-400">Advanced Log Analysis & Threat Detection</p>
        </div>

        {/* Register Card */}
        <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-2xl p-8">
          <h2 className="text-2xl font-bold text-white mb-2">Create Account</h2>
          <p className="text-slate-400 text-sm mb-6">Get started with LogLens today</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center p-3 bg-slate-700 rounded-lg">
              ...
              />
              <label htmlFor="isPremium" className="ml-3 text-sm font-medium text-slate-200 cursor-pointer flex-1">
                Start with Pro Tier
              </label>
              <span className="text-xs text-slate-400">$9.99/mo</span>
            </div>
